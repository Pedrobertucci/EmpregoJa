...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
   ...
   if(!empty($operation)){
      ...
      if($operation == 'register'){
         ...
         if(isset($data -> user ) && !empty($data -> user) && isset($data -> user -> name)
            && isset($data -> user -> sobrenome) && isset($data -> user -> email) && isset($data -> user -> password)){
          ...
          if ($fun -> isEmailValid($email)) {
            ...
            echo $fun -> registerUser($name, $sobrenome, $email, $password);
  ...
  echo "Pedro Bertucci API Emprego já iluminati ";
